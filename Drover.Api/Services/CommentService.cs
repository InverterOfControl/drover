...
namespace Drover.Api.Services
{
    public class CommentService : BaseService, ICommentService
  {
    ...
    public async Task<Comment> CreateComment(long projectId, long taskId, string user, string text)
    {
      var newComment = new Comment { Email = user, Text = text };
      var request = new CreateCommentRequest { ProjectId = projectId, TaskId = taskId };

      var response = await _api.CreateComment(request, newComment);

      return response.Comment;
    }

    public async Task<IList<Comment>> GetComments(long projectId, long taskId)
    {
      var request = new ListCommentRequest { ProjectId = projectId, TaskId=taskId };

      var response = await _api.ListComments(request);
